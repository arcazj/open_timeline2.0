...
export class ConfigurationFields {
  ...
  render() {
    ...
    if (this.family === 'filters') {
      ...
      this.filterEditor = new FilterEditor(this.element.querySelector('.cfg-filter-builder'), { expression: value.expression, searchMode: value.search.mode, searchCaseSensitive: value.search.caseSensitive, searchFields: value.search.fields, updateIcons: this.updateIcons, ...(this.fieldTypes ? { fieldTypes: this.fieldTypes } : {}) });
    }
  ...
  value({ validate = true } = {}) {
    ...
    if (this.family === 'views') {
      ...
      for (const key of ['range', 'overview']) if (this.element.querySelector(`[data-override="${key}"]`).checked) value.settings[key] = { from: inputIso(node(`${key}-from`).value), to: inputIso(node(`${key}-to`).value) }; else delete value.settings[key];
