...
export function validateSharedView(value) {
  keys(value, ['version', 'range', 'domain', 'settings', 'filters', 'search', 'view', 'scaleStrategy', 'selectedId', 'generation']);
  if (value.version !== 1) fail('Unsupported shared-view version');
  ...
  keys(value.search, ['search', 'searchMode', 'searchCaseSensitive', 'searchFields']);
  if (!text(value.search.search, 4096) || !['any', 'all', 'phrase'].includes(value.search.searchMode) || typeof value.search.searchCaseSensitive !== 'boolean') fail('Invalid shared search');
